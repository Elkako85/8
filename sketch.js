...
var particles_b = [];
var particles_c = [];
var fade = 20; // the length of the particle tail
var radius = 5; // radius of the particles
...
function setup() {
  ...
  for (let i=0; i<num; i++) {
    ...
    //particles[i]= new Particle(loc, dir, speed); slower particles will looks shorter 
		particles_a[i] = new Particle(loc_a, dir_a, 0.7);
...
function draw() {
  ...
  fill(0, 3); // put a hight value for hiding the tails 
  noStroke();
  ...
  for (let i=0; i<num; i++) {
		...
		fill(220, 80, 19, fade); //turqouise
		particles_b[i].move();
		particles_b[i].update(radius);
		particles_b[i].checkEdges();

		fill(60, 110, 60, fade); // blue
		particles_c[i].move();
